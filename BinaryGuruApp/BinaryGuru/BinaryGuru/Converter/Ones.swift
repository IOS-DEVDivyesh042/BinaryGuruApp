...
class Ones : UIViewController {
    ...
    private let complementButton: UIButton = {
        let button = UIButton()
        button.setTitle("Ones Complement", for: .normal)
    ...
    private func setupViews() {
        ...
        complementButton.addTarget(self, action: #selector(calculateOnesComplement), for: .touchUpInside)
    }
    ...
    @objc private func calculateOnesComplement() {
        ...
        let onesComplement = ~binaryNumber
        resultLabel.text = "One's Complement: \(String(onesComplement, radix: 2))"
    }
}
